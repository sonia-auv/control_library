function [zopt,exitflag] = LinearNlMpcSolver(H, f, A, b, Aeq, beq, LB, UB, z0)
%function [zopt,cost,exitflag] = LinearNlMpcSolver(FUN,z0,A,b,Aeq,beq,lb,ub,NONLINCON)
...
%% Set dimensional information of linear and nonlinear constraints
 %options = optimoptions('linprog','Algorithm','dual-simplex','OptimalityTolerance',0.02,...
                        %'ConstraintTolerance',0.02);
 Opt = optimoptions('quadprog', ...
                    'Display', 'none', ...
                    'ConstraintTolerance', 0.02, ...
                    'OptimalityTolerance', 0.02, ...
                    'StepTolerance', 0.1, ...
                    'MaxIterations', 50); 
                
[zopt,~, exitflag,~] = quadprog(H, f, A, b, Aeq, beq, LB, UB, z0,Opt);                        
% f=FUN*NONLINCON;
% [zopt,cost,exitflag,~] = linprog(f,A,b,Aeq,beq,lb,ub);
% disp(A)
