...
void *baseRateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    // External mode
    {
      ...
      rtExtModePauseIfNeeded(proc_control_node_M->extModeInfo, 6, &rtmStopReq);
      if (rtmStopReq) {
...
void *backgroundTask(void *arg)
{
  while (runModel) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(proc_control_node_M->extModeInfo, 6, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(proc_control_node_M, true);
      }
    }
...
int main(int argc, char **argv)
{
  ...
  rtExtModeCheckInit(6);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(proc_control_node_M->extModeInfo, 6, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(proc_control_node_M, true);
    }
  }
